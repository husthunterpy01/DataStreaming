...
redisStreamingDF.withColumn("value",from_json("value",kafkaRedisSchema)) \
...
# Be sure to specify the option that reads all the events from the topic including those that were published before you started the spark stream
stediEventRawStreamingDF = spark.readStream \
                        .format("kafka") \
                        .option("kafka.bootstrap.servers", "localhost:9092") \
...
# TO-DO: cast the value column in the streaming dataframe as a STRING 
stediEventStreamingDF = stediEventRawStreamingDF.selectExpr("cast(key as string) key", "cast(value as string) value")
...
redisStreamingDF.withColumn("value",from_json("value",customerRiskSchema)) \
                .select(col("value.*")) \
...
query = customerRiskAndBirthYearDF.selectExpr("cast(customer as string) key","to_json(struct(*)) as value") \
                                  .writeStream \
                                  .format("kafka") \
                                  .option("kafka.bootstrap.servers", "localhost:9092") \
