...
encodedStreamingDF = spark.sql("select key,zSetEntries[0].element as encodedCustomer from RedisSortedSet")
...
zSetEntriesDecodingStreamingDF = encodedStreamingDF \
...
zSetEntriesDecodingStreamingDF.withColumn("customer", from_json("customer",customerJsonSchema)) \
                              .select(col("customer.*")) \
